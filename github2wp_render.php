<?php
					...
					foreach($commit_history as $commit) {
						...
						?>
								...
								<tr class="<?php echo ($k % 2) ? 's-inactive' : '';?>">
									...
									<td width="10%;"><input type='submit' value='<?php _e('Revert', GITHUB2WP);?>' class="downgrade button-secondary" id="downgrade-resource-<?php echo $resource_id."-".$commit['sha']; ?>" /></td>
								</tr>
	...
	function github2wp_return_resource_dismiss($resource, $index) {
		if($index != 0) {
			...
			return '<p><input name="submit_delete_resource_'.$index
					.'" type="submit" class="button button-red btn-medium" value="'.__('Dismiss', GITHUB2WP)
					.'" onclick="return confirm(\''.__('Do you really want to disconect from Github:', GITHUB2WP)
	...
	function github2wp_render_tab_menu( $tab ) {
		...
		<h2 class="nav-tab-wrapper">
			...
			echo' nav-tab-active';?>" href="<?php echo github2wp_return_settings_link('&tab=resources'); ?>"><?php _e('Resources', GITHUB2WP);?></a>
			<a class="nav-tab<?php if($tab=='history')
			echo' nav-tab-active';?>" href="<?php echo github2wp_return_settings_link('&tab=history'); ?>"><?php _e('History', GITHUB2WP);?></a>
			<a class="nav-tab<?php if($tab=='settings')
			echo' nav-tab-active';?>" href="<?php echo github2wp_return_settings_link('&tab=settings'); ?>"><?php _e('Settings', GITHUB2WP);?></a>
		</h2>
		<?php
	}
	...
	function github2wp_render_resource_form() {
		...
		<input name="submit_resource" <?php echo $disable_resource_fields; ?>type="submit" class="button button-primary" value="<?php _e('Add Resource', GITHUB2WP); ?>" />
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">
							...
							<td>
								<label><a href="https://github.com/settings/applications/new" target="_blank"><?php _e('Create a new git application', GITHUB2WP);?></a></label>
								<p class="description"><strong><?php _e('Application Name', GITHUB2WP);?></strong> -> git2wp</p>
			...
			<table class="form-table">
				<tbody>
					...
					<?php else:
						...
						<tr valign='top'>
							<th scope='row'>
								<label><?php _e('GitHub Link Status:', GITHUB2WP);?> </lablel>
							</th>
							<td>
