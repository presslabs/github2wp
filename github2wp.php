...
require_once('GitHub2WP.class.php');
require_once('GitHub2WPFile.class.php');
...
add_filter("pre_set_site_transient_update_themes","github2wp_update_check_themes", 10, 1); //WP 3.0+
...
function github2wp_ajax_callback() {
	...
	if( isset($_POST['github2wp_action']) and $_POST['github2wp_action'] == 'set_branch' ) {
	...
	if( isset($_POST['github2wp_action']) and  $_POST['github2wp_action'] == 'downgrade' ) {
		if( isset($_POST['commit_id']) and isset($_POST['res_id'])) {
			...
			if($sw) {
				...
				$response['error_message'] = sprintf(__("The resource <b>%s<b> has FAILED to updated to %s .", GITHUB2WP), $resource['repo_name'], $version);
	...
	if( isset($_POST['github2wp_action']) and $_POST['github2wp_action'] == 'fetch_history' ) {
...
function github2wp_rmdir($dir) {
	...
	foreach ( scandir($dir) as $item ) {
		...
		if ( ! github2wp_rmdir($dir . "/" . $item) ) {
			chmod($dir . "/" . $item, 0777);
			if ( ! github2wp_rmdir($dir . "/" . $item) ) return false;
		}
	}
	return rmdir($dir);
}
...
function github2wp_uploadThemeFile($path, $mode = 'install') {
	...
	$file = new GitHub2WPFile($ftr, $ftw);
...
<table id="the-list" class="wp-list-table widefat plugins" cellpadding='5' border='1' cellspacing='0' >
	<thead>
		<tr><th></th><th><?php _e('Resource', GITHUB2WP);?></th><th><?php _e('Options', GITHUB2WP);?></th></tr>
	</thead>
	<tbody>
<?php 
		...
		if(is_array($resource_list) && !empty($resource_list))		
			foreach($resource_list as $index => $resource) {
				...
				$action = github2wp_return_resource_dismiss($resource, $k-1); // CHANGE HERE AFTER UPDATE INSTALL DELETE USES INDEX
				
				if ( ! $dir_exists ) {
					$my_data .= "<p><strong>".__('The resource does not exist on your site!', GITHUB2WP)."</strong></p>";
					$action .= github2wp_return_resource_install($resource, $k-1); // CHANGE HERE AFTER UPDATE INSTALL DELETE USES INDEX
				}
				...
				if ( $repo_type == 'plugin' ) {
					...
					if ( ($new_version != $current_plugin_version) && ('-' != $current_plugin_version) && ('' != $current_plugin_version)  && ($new_version != false) ) {
						...
						$action .= github2wp_return_resource_update($resource, $k-1); // CHANGE HERE AFTER UPDATE INSTALL DELETE USES INDEX
					}
				}
				else
					if ( $repo_type == 'theme' ) {							
...
function github2wp_options_validate($input) {
	...
	if( isset($_POST['submit_resource']) && !github2wp_needs_configuration() ) {
		...
		if ($repo_link != '') {
			...
			$data = GitHub2WP::get_data_from_git_clone_link($repo_link);
			...
			if(isset($data['user']) && isset($data['repo'])) {
				...
				if($unique) {
					...
					if ($sw) {
						$on_wp = GitHub2WP::check_svn_avail($resource_repo_name, substr($_POST['resource_type_dropdown'], 0, -1));
						$head = $git->get_head_commit();
...
function github2wp_init() {
	...
	if ( isset($_GET['code']) and  isset($_GET['github2wp_auth']) and $_GET['github2wp_auth'] == 'true' ) {
